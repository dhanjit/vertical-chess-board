...
- [ ] Print the pivot test print for your chosen `pivot_type` (**D12**).
      ...
      **pivot test coupon** instead (there are no printed pivot parts in that
      ...
      Under `"bearing"`: the same hub + cap pair as `"pin"`, plus one bought
      **MR63ZZ bearing** — and its first question is not lean but **ring-down
      time** after a flip (nothing damps the swing; see
...
- ✅ **Parametric hardware** — pieces, gravity gimbal, board panel, frame,
  ...
  renders every variant combination). **Two piece styles × three pivot
  architectures** (**D12**) all render clean at 55 mm squares; every one of the
  36 piece/style/pivot combinations exports as a single shell, and the tables
  above are measured off those meshes (the `"bearing"` pivot's parking figure
  is an estimate, not a mesh measurement — see
  [`PIECE_DESIGNS.md`](PIECE_DESIGNS.md)).
